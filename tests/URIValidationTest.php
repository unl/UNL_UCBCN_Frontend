...
{
    public $uri = 'http://localhost/events/';
    ...
    {
        $r = $this->validator->validateFragment(file_get_contents($this->uri));
        $this->assertEquals(get_class($r), 'Services_W3C_HTMLValidator_Response');
        if (!$r->isValid()) {
            $message = $this->constructErrorMessage($r);
        } else {
    ...
    {
        $message = $this->uri.' is NOT VALID: '.count($r->errors).' Errors -'.PHP_EOL;
